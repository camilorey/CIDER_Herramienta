 ...
 * @author laptop
 */
public class CIDER_DB_EntityHandler {
 protected HashMap<String, CIDER_DB_Entity> entities;
 public CIDER_DB_EntityHandler() {
 ...
 public int entitySatisfiesQuery(String entityName, String query){
  if(!isIn(entityName)){
   return -3;
    ...
    if(!(get(entityName) instanceof CIDER_Centro)){
     return -2;
    }else{
     CIDER_Centro centro = (CIDER_Centro) get(entityName);
     String[] queryTokens = query.split(",");
     boolean response = true;
     for(int i=0;i<queryTokens.length;i++){
      if(!queryTokens[i].equals(""))
       response = response && centro.respondToQuery(queryTokens[i]);
     }
     if(response)
      return 1;
     else
